...
def get_args_parser():
    ...
    parser.add_argument('--snnet_name', default='snnet', type=str, help='configuration')
...
def main(args):
    ...
    for i, anchor_name in enumerate(args.anchors):
        ...
        anchor = create_model(
            anchor_name,
            pretrained=True,
            pretrained_deit=args.pretrained_deit,
    ...
    if args.snnet_name == 'snnet_v1':
        model = SNNet(anchors)
    elif args.snnet_name == 'snnet_v2':
        model = SNNetv2(anchors, lora_r=args.lora_r)

    if args.low_rank:
        for name, param in model.named_parameters():
            if 'stitch_layers' in name and 'transform' in name:
                param.requires_grad = False
                logger.info('freeze %s' % name)
    ...
    if args.attn_only:
        ...
        try:
            ...
            logger.info('no position encoding')
        try:
            for p in model.patch_embed.parameters():
                p.requires_grad = False
        except:
            logger.info('no patch embed')
    ...
    grouped_subnet = group_subnets_by_flops(flops_params, flops_gap=args.flops_gap)

    model.flops_grouped_cfgs = grouped_subnet
    if args.flops_sampling_k:
        y = [(1 / args.flops_sampling_k) * i + 1 / len(grouped_subnet) for i in list(range(len(grouped_subnet)))]
        y = np.array(y)
        y = y / np.sum(y)
        model.flops_sampling_probs = y
    else:
        model.flops_sampling_probs = np.ones(len(grouped_subnet)) / len(grouped_subnet)
