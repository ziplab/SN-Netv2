...
def multi_gpu_test_snnet(model,
                   data_loader,
                   tmpdir=None,
                   gpu_collect=False,
    ...
    for i, cfg_id in enumerate(all_cfgs):
        print('Testing: {}/{}'.format(i+1, len(all_cfgs)))
        if rank == 0:
            prog_bar = mmcv.ProgressBar(len(dataset))
        if hasattr(model, 'module'):
        ...
        for batch_indices, data in zip(loader_indices, data_loader):
            ...
            # print('Memory: {:.2f} MB'.format(torch.cuda.memory_allocated() / MB))
